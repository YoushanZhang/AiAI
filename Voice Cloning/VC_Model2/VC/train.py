...
#----------------------------------------#
from model.cus_model import Cus_model
...
def save_states(global_step, mel_outputs, linear_outputs, attn, y,
	...
	plot_alignment(alignment.T, path, info="Model, step={}".format(global_step))
...
def model_step(model, optimizer, criterion,
...
def train(model, 
	...
	while global_epoch < nepochs:
		...
		for x, input_lengths, mel, y in data_loader:
			...
			model, optimizer, Ms, Rs = model_step(model, optimizer, criterion,
												 x, input_lengths, mel, y,
												 init_lr, sample_rate, clip_thresh,
...
def initialize_training(checkpoint_path):
	...
	# Model
	model = Cus_model(n_vocab=len(symbols),
