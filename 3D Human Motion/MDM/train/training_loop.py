...
class TrainLoop:
    def __init__(self, args, train_platform, model, diffusion, data):
        ...
        self.scheduler = ReduceLROnPlateau(self.opt, 'min', factor=0.5, patience=10) # newly added
    ...
    def _load_and_sync_parameters(self):
        ...
        if resume_checkpoint:
            ...
            checkpoint = torch.load(resume_checkpoint, map_location=dist_util.dev())
            filtered_checkpoint = {k: v for k, v in checkpoint.items() if 'clip_model' not in k}
    
            # newly added check
            for param in filtered_checkpoint.values():
                assert not torch.isnan(param).any(), "NaN values found in model parameters after loading state dict"
                assert not torch.isinf(param).any(), "Inf values found in model parameters after loading state dict"
            
            
            self.model.load_state_dict(filtered_checkpoint, strict=False)  # Use strict=False to ignore non-matching keys
