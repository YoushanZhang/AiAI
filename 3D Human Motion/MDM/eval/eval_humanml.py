...
def evaluate_diversity(activation_dict, file, diversity_times):
    ...
    for model_name, motion_embeddings in activation_dict.items():
        # Dynamically adjust diversity_times based on the number of available samples
        actual_num_samples = motion_embeddings.shape[0]
        adjusted_diversity_times = min(diversity_times, actual_num_samples - 1)
        
        # Log the actual number of samples and the adjusted diversity_times
        logging.info(f"[{model_name}] Number of samples available for diversity calculation: {actual_num_samples}")
        logging.info(f"[{model_name}] Adjusted diversity_times value: {adjusted_diversity_times}")
        
        # Proceed with diversity calculation if enough samples are available
        if actual_num_samples > 1:  # Ensure there's more than one sample to calculate diversity
            diversity = calculate_diversity(motion_embeddings, adjusted_diversity_times)
            eval_dict[model_name] = diversity
            print(f'---> [{model_name}] Diversity: {diversity:.4f}')
            print(f'---> [{model_name}] Diversity: {diversity:.4f}', file=file, flush=True)
        else:
            logging.warning(f"[{model_name}] Not enough samples generated for diversity calculation. Skipping diversity calculation for this model.")
            eval_dict[model_name] = None  # Or set to a default/indicative value as appropriate
    return eval_dict
