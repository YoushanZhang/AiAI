...
import models.vqvae_imp as vqvae
...
# Determine dataset-specific configurations
...
# Get evaluation wrapper options
...
train_loader = dataset_VQ.DATALoader(
    ...
    num_workers=4,  # Use multiple workers for better data loading performance
...
progress_bar = tqdm(range(1, args.warm_up_iter + 1), desc="Warm-up", leave=False)  # TQDM for progress

for nb_iter in progress_bar:
    optimizer, current_lr = update_lr_warm_up(optimizer, nb_iter, args.warm_up_iter, args.lr)
    
    gt_motion = next(train_loader_iter).cuda().float()  # (bs, 64, dim)
...
# ----- Main Training Loop -----
avg_recons, avg_perplexity, avg_commit = 0.0, 0.0, 0.0
progress_bar = tqdm(range(1, args.total_iter + 1), desc="Training", leave=True)
...
for nb_iter in progress_bar:
    gt_motion = next(train_loader_iter).cuda().float()  # (bs, nb_joints, joints_dim, seq_len)
