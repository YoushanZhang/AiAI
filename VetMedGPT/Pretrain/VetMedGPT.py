...
model_name = "Vet_Med_GPT_1b"
name = "pretrain"
out_dir = Path("/scratch/vetgpt/out") / name
...
TBlogger = TensorBoardLogger(root_dir=("logs"), name="tensorboard")
...
def setup(
    ...
    fabric = L.Fabric(devices=devices, strategy=strategy, precision=precision, loggers=[logger,TBlogger])
...
def main(fabric, train_data_dir, val_data_dir, resume):
    ...
    # fabric.load(checkpoint_path, model, strict=True)
    ...
    for  train_data in train_dataloader:
        ...
        if resume:
            if curr_iter < initial_iter:
                fabric.print(f"resuming from iter {curr_iter}", end="\r")
                curr_iter += 1
                continue
            else:
                resume = False
